...
class OpenAICompatibleLLMAdapter:
    ...
    def suggest(self, request: LLMFallbackRequest) -> LLMFallbackSuggestion | None:
        ...
        return self._parse_response(request, response_text, prompt_hash, started)
...
class LLMFallbackService:
    ...
    @staticmethod
    def _evidence(suggestion: LLMFallbackSuggestion) -> list[str]:
        evidence = [
            "LLM fallback suggestion requires human review",
            f"adapter_model={suggestion.model}",
            f"latency_ms={suggestion.latency_ms}",
            f"prompt_hash={suggestion.prompt_hash}",
            f"response_hash={suggestion.response_hash}",
            f"reason={suggestion.reason}",
        ]
        if suggestion.error_code:
            evidence.append(f"error_code={suggestion.error_code}")
        return evidence
