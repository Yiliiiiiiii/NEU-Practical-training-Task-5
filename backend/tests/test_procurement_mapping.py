...
def test_procurement_aliases_map_project_budget_and_award_without_crossing() -> None:
    ...
    assert mappings["award_amount"]["source_field"]["source_name"] == "成交金额"
    assert mappings["budget_amount"]["source_path"] != mappings["award_amount"]["source_path"]
    assert mappings["budget_amount"]["status"] == "accepted"
    assert mappings["award_amount"]["status"] == "accepted"
...
def test_ambiguous_procurement_amounts_are_not_forced_into_accepted_mappings() -> None:
    ...
    accepted_amount_targets = {
        ...
        if item["target_field_id"] in {"budget_amount", "award_amount"}
    }
    ambiguous_reviews = [
        item
...
def test_procurement_datetime_transform_validation_and_package_verification(
    ...
    assert transform_result.data["procurement_type"] == "public_tender"
    assert transform_result.data["budget_amount"] == 1200000.0
    assert transform_result.data["bid_deadline"] == "2026-07-15"
...
def test_procurement_invalid_datetime_is_rejected_during_transform() -> None:
    ...
    assert "bid_deadline" not in result.data
    assert any(
        error["field_id"] == "bid_deadline" and error["code"] == "date_format_error"
