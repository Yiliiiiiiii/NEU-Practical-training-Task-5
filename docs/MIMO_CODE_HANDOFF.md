...
- 已完成 Phase 0 至 Phase 4。
- 当前功能基线提交为 `56b0b26 feat: add candidate extraction and rule mapping`。
- 当前功能分支为 `phase4-candidates-mapping`。
- 基线验证为 `43 passed`，Ruff 全部通过。
- `main` 尚不包含 Phase 3 和 Phase 4。**不得从旧的 `main` 开始 Phase 5。**
- Phase 5 必须从包含 `56b0b26` 以及本交接文档的最新 `HEAD` 创建分支。

如果基线测试失败、工作区存在来源不明的修改、当前提交不包含 `56b0b26`，立即停止并报告，不要猜测或覆盖文件。
...
```powershell
git switch -c phase5-transform-canonical
```

Phase 5 完成并汇报后必须停止。未经用户或 Codex 验收通过，不得开始 Phase 6。

后续阶段依次为：

1. Phase 5：字段转换与 canonical model
2. Phase 6：多形态渲染
3. Phase 7：成果包与一致性校验
4. Phase 8：前端最小页面
5. Phase 9：测试与稳定化
6. Phase 10：验收强化与产品化基线

不得跨 Phase 提前实现功能。例如 Phase 5 不实现渲染、ZIP、前端或真实模型接入。
...
- Transform、canonical、render、validate、manifest、ZIP 和 frontend 当前尚未实现。
...
现在从完整阅读 `SchemaPack_Agent_项目实施文档_修订版.md` 开始，然后只制定并执行 Phase 5 计划。
