...
## Hard-Gap Batch 1 Trace Coverage

The graph includes explicit nodes and edges for metadata source to document metadata field,
source block to extractive summary sentence, configured tag trace and rule evidence to
chunk, upstream entity to chunk entity tag, UIR/canonical block to Markdown block,
canonical field to `content.json` structured field, and chunk to source blocks.

Lineage summaries expose `metadata_source_coverage`, `summary_source_coverage`,
`tag_trace_coverage`, `entity_source_coverage`, and `markdown_block_coverage`. These are
trace coverage metrics only; they do not score semantic quality.
