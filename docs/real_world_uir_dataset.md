...
当前扩展后的数据集包含 45 个 UIR documents，覆盖 `general_doc`、`meeting_doc`、`policy_doc` 和 `procurement_doc`。其中非采购 recall 子集为 35 个 documents：
...
2026-07-03 三项深化后的提交证据记录：

- dataset size：45；
- imports：45/45；
- task executions：45/45；
- package verification：45/45；
- real-world mapping badcase violations：0；
- real-world mapping recall：`0.5672514619883041`；
- 非采购 API-backed evaluator：35/35 package verification，badcase violations 0，average recall `0.5677551020408163`，review-required 69，required missing 6；
- 新增 15 个样本全部通过 strict UIR validation，且包含 source manifest、mapping gold、badcase、retrieval query 与 review fixture 证据。
