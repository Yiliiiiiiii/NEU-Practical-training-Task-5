...
| 检查 | 当前结果 | 复现命令 |
| --- | --- | --- |
| backend pytest | 885 passed | `backend\.venv\Scripts\python.exe scripts\verify_all.py --check-openapi` |
| backend ruff | clean | 同上 |
| frontend build | successful | 同上 |
| OpenAPI export/check | 65 paths | 同上 |
...
## Topic 5 Phase 3 SchemaPack Contract Acceptance
...
Acceptance evidence is generated by `scripts/eval_schema_pack_contracts.py` and `scripts/check_schema_pack_contract_gate.py`. Both `announcement_doc` and `event_notice_doc` include manifests, output assertions, positive fixtures, and badcases. Assertion reports are task-side evidence by default and optional checksummed Package 1.1 entries when explicitly enabled.
...
Feature-aware Package 1.1 outputs now prove metadata-template effects, configured tags,
local quality tags, extractive summary faithfulness, upstream entity passthrough,
Topic 11 fallback/invalid-output rejection, deterministic Markdown anchors, consistency
tamper detection, final manifest checksums, and legacy compatibility. Announcement and
event-notice golden packages pass strict verification and three-run semantic hashes match.

This acceptance is limited to deterministic Topic 5 conversion and traceability. It does
not expand the claim to raw-file parsing, OCR, embeddings, RAG, Topic 6 quality scoring,
publication routing, or arbitrary production shadow/blind semantic recall.
